# -*- coding: utf-8 -*-
"""
Eğitim v4 – Bellek Optimizasyonlu (Benign limitli, sıralı okuma)
"""
...
MAX_BENIGN = 3_000_000            # Benign akış üst sınırı (bellek için)
MAX_PER_ATTACK_CLASS = 2_000_000  # Saldırı sınıfları için toplam üst sınır
...
def load_all_data():
    ...
    for label_name, paths in EVE_FILES.items():
        ...
        if isinstance(paths, list):
            per_file = max_for_class // len(paths)
            file_list = paths
        else:
            per_file = max_for_class
            file_list = [paths]
...
def train():
    print("=" * 60)
    print("IDS EĞİTİM v4 – Bellek Optimizasyonlu")
    print(f"Feature sayısı : {N_FEATURES}")
    print(f"Sınıflar       : {list(CLASS_MAP.keys())}")
    print(f"Benign üst sınır: {MAX_BENIGN:,}")
    ...
    bundle = {
        ...
        "trained_on"   : "CICIDS2018-eve.json + MAWI backbone (6-class optimized)",
