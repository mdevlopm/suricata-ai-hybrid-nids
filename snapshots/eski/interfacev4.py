...
# -*- coding: utf-8 -*-
"""
inference_v4.py – Eğitim v4 ile uyumlu çıkarım motoru
Kullanım:
  Gerçek zamanlı: python3 inference_v4.py --eve /var/log/suricata/eve.json
  Batch:         python3 inference_v4.py --eve test.json --batch --output ./sonuc.json
"""
...
# ═══════════════ ÖZELLİK ÇIKARIMI (v4 ile AYNI) ═══════════════
def extract_features(event: dict):
    """Eğitim v4'teki extract_features fonksiyonunun aynısı"""
        ...
        with open(eve_path, encoding="utf-8", errors="replace") as f:
            for line in f:
                ...
                if stats["total"] % 100_000 == 0:
                    ...
                    print(f"  İşlenen: {stats['total']:,}  Saldırı: {stats['attack']:,}  Benign: {stats['benign']:,}  Süre: {elapsed:.1f} sn")
                    if attack_types:
                        print("  ── Saldırı türleri ──")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="IDS AI Çıkarım Motoru v4")
    ...
    parser.add_argument("--threshold", type=float, default=None, help="Özel eşik değeri")
    parser.add_argument("--batch", action="store_true", help="Batch mod")
