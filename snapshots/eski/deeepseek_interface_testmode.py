...
# -*- coding: utf-8 -*-
"""
deepseekinterface.py - Eve.json IDS Çıkarımı (egitim_v2.py ile uyumlu)
=========================================================================
Özellikler:
  • Gerçek zamanlı (tail) veya batch mod
  • Her 100.000 flow'da detaylı rapor (saldırı türleriyle birlikte)
  • Hiçbir duraklama / Enter bekleme yok
  • Eğitim kodundaki feature extraction'ın birebir aynısını kullanır

Kullanım:
  Gerçek zamanlı:
    python3 deepseekinterface.py --eve /var/log/suricata/eve.json

  Batch:
    python3 deepseekinterface.py --eve test.eve.json --batch --output ./alerts.json
"""
...
# ── FEATURE EXTRACTION (egitim_v2.py ile BİREBİR AYNI) ────────────────────
def extract_features(event: dict):
    """Eve.json flow event'inden tam 42 özellik çıkarır."""
...
def load_model(path: Path):
    """Eğitim bundle'ını açar, model + scaler + threshold + etiket haritası döner."""
...
def process_event(event, model, scaler, threshold, inv_class_map):
    """Flow event'ini sınıflandırır, eşik üstündeyse alert sözlüğü döndürür."""
    features, meta = extract_features(event)
    if features is None:
        return None

    prob     = model.predict_proba(scaler.transform(features))[0]
    prob_atk = 1.0 - prob[0]                       # Sınıf 0 = Benign
...
def run_batch(eve_path, model, scaler, threshold, inv_class_map, output_path):
    ...
    attack_types = {}          # anlık saldırı türü sayacı
        ...
        with open(eve_path, encoding="utf-8", errors="replace") as f:
            for line in f:
                ...
                if alert:
                    ...
                    # İsterseniz her alert'i yazdırmak için aşağıdaki satırı aktif edin
                    # print(f"  [ALERT] {alert['timestamp']} | {alert['src_ip']} -> {alert['dest_ip']}:{alert['dest_port']} | {lbl} | conf={alert['ai']['confidence']:.3f}")
                else:
                    stats["benign"] += 1

                # ═══════════════ 100.000 flow'da bir DETAYLI RAPOR ═══════════════
                if stats["total"] % 100_000 == 0:
                    elapsed = time.time() - start_time
                    print(f"\n{'─' * 55}")
                    print(f"  RAPOR (her 100.000 flow)")
                    print(f"  İşlenen flow     : {stats['total']:,}")
                    print(f"  Toplam saldırı   : {stats['attack']:,}")
                    print(f"  Benign           : {stats['benign']:,}")
                    print(f"  Geçersiz satır   : {stats['skip']:,}")
                    print(f"  Geçen süre       : {elapsed:.1f} sn")
                    if attack_types:
                        print(f"  ── Tespit edilen saldırı türleri ──")
                        for lbl, cnt in sorted(attack_types.items(), key=lambda x: -x[1]):
                            print(f"    {lbl:<20} : {cnt:,}")
                    print(f"{'─' * 55}\n")
                    out.flush()   # tamponu diske yaz
...
# ── GERÇEK ZAMANLI MOD ─────────────────────────────────────────────────────
class EveJsonTailer:
    """Suricata eve.json dosyasını log rotasyonunu da takip ederek okur."""
        ...
        while True:
            for line in tailer.readlines():
                ...
                if alert:
                    ...
                    out.write(json.dumps(alert) + "\n")
                    out.flush()
                    # Her alert'i yazdırmak isterseniz aşağıdaki satırı aktif edin
                    # print(f"[ALERT] {alert['timestamp']} | {alert['src_ip']} -> {alert['dest_ip']}:{alert['dest_port']} | {lbl} | conf={alert['ai']['confidence']:.3f}")
                else:
                    stats["benign"] += 1

                # Gerçek zamanda 100k'da bir rapor
                if stats["total"] % 100_000 == 0:
                    elapsed = time.time() - start_time
                    print(f"\n{'─' * 55}")
                    print(f"  GERÇEK ZAMANLI RAPOR (her 100.000 flow)")
                    print(f"  İşlenen flow     : {stats['total']:,}")
                    print(f"  Toplam saldırı   : {stats['attack']:,}")
                    print(f"  Benign           : {stats['benign']:,}")
                    print(f"  Geçen süre       : {elapsed:.1f} sn")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="IDS AI çıkarım motoru")
    parser.add_argument("--eve",       default=str(DEFAULT_EVE), help="eve.json yolu")
    parser.add_argument("--model",     default=str(DEFAULT_MODEL), help="Model dosyası (.pkl)")
    parser.add_argument("--output",    default=str(DEFAULT_OUT), help="Alert çıktı dosyası")
    parser.add_argument("--threshold", type=float, default=None, help="Özel eşik değeri (varsayılan: eğitimden gelen)")
    parser.add_argument("--batch",     action="store_true", help="Batch mod (tek seferlik işle)")
