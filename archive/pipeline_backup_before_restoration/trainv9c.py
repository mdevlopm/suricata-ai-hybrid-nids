# -*- coding: utf-8 -*-
"""
trainv9c.py — XGBoost IDS Training with CTU-13 Bot Addition & Standardized Evaluation
=====================================================================================
ISOLATING THE EFFECT OF CTU-13 BOT FLOWS:
  1. Supervised Source Dataset: CICIDS2018 (relabeled) + CTU-13 Bot (18 flows) -> Total Bot = 40,857 valid flows
  2. CORAL Target Domain: SADECE CICIDS2017 Thursday (50,000 flows from target EVE file). NO MCFP.
  3. Hyperparameters: IDENTICAL to trainv8.py (max_depth=8, eta=0.05, subsample=0.80, colsample_bytree=0.80, min_child_weight=3)
  4. Standardized Evaluation Population: TAM 334,783 akışlık data/eve/eve.json dosyası üzerinde FP Oranı ölçümü!
  5. Benchmark Evaluation: StratifiedGroupKFold(group=src_ip) 5-fold Macro F1 + Thursday 334.783 FP Rate
"""
...
log = logging.getLogger("trainv9c")
...
DEFAULT_OUT_MODEL = Path("./ids_model_v9c_final.pkl")
...
def main(args):
    log.info("=" * 60)
    log.info("TRAINV9C — XGBoost + CTU-13 Supervised + Pure Thursday CORAL + Standard EVE Evaluation")
    log.info("=" * 60)
    ...
    log.info("\n[6/6] Optimizing binary threshold and evaluating FP rate on FULL 334,783 flows (data/eve/eve.json)...")
    ...
    if args.target_eve.exists():
        ...
        with open(args.target_eve, "r", encoding="utf-8", errors="replace") as f:
            for line in f:
                ...
                fv = extract_features_v7(ev)
                if fv is not None: t_feats.append(fv)
    ...
    log.info("\n" + "=" * 80)
    log.info("  TRAINV9C FINAL VERIFIED SUMMARY")
    log.info("=" * 80)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="trainv9c — XGBoost + CTU-13 Supervised + Pure Thursday CORAL")
