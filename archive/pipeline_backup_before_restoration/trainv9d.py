# -*- coding: utf-8 -*-
"""
trainv9d.py — XGBoost IDS Training (CTU-13 Supervised + Pure Thursday CORAL + NO CLASS WEIGHTS)
==============================================================================================
ISOLATING THE EFFECT OF CLASS WEIGHTS:
  1. Identical to trainv9c.py in every single aspect.
  2. SINGLE ISOLATION CHANGE: Omit sample_weight (weight=None) in xgb.DMatrix (v8 original method).
  3. Chunked Low-RAM Evaluation: Evaluate FP rate in 50,000 flow batches (< 100 MB RAM).
"""
...
DEFAULT_TARGET_EVE = Path("./data/eve/eve.json")  # 334,783 flows full file!
DEFAULT_OUT_MODEL = Path("./ids_model_v9d_final.pkl")
...
def load_labeled_data(data_root: Path, ctu13_bot_path: Path, max_per_class: dict):
    ...
    if ctu13_bot_path.exists():
        ...
        log.info(f"  Loaded CTU-13 Bot  : {len(ctu_feats):>8,} samples (Total Bot = {40839 + len(ctu_feats):,})")

    X = np.vstack(X_list)
    y = np.concatenate(y_list)
    return X, y, np.array(src_ips)

def load_unlabeled_target(thursday_path: Path, max_thursday: int):
    t_feats = []
    if thursday_path.exists():
        ...
        log.info(f"  Target CORAL Thursday ONLY: {len(t_feats):,} samples (from full EVE file)")

    return np.array(t_feats, dtype=FEATURE_DTYPE)
...
def evaluate_fp_rate_chunked(bst, scaler, coral, best_thr, eve_path: Path, chunk_size=50_000):
    """Memory-efficient chunked FP rate evaluation (< 100 MB RAM)."""
    total_flows = 0
    fp_flows = 0
    chunk_feats = []
    ...
    with open(eve_path, "r", encoding="utf-8", errors="replace") as f:
        for line in f:
            ...
            fv = extract_features_v7(ev)
            if fv is not None:
                chunk_feats.append(fv)
                
            if len(chunk_feats) >= chunk_size:
                X_chunk = np.array(chunk_feats, dtype=FEATURE_DTYPE)
                X_corr = coral.transform_target(X_chunk)
                X_s = scaler.transform(X_corr)
                probs = bst.predict(xgb.DMatrix(X_s))
                atk_prob = 1.0 - probs[:, 0]
                
                total_flows += len(chunk_feats)
                fp_flows += int((atk_prob >= best_thr).sum())
                chunk_feats = []
                gc.collect()
    ...
    fp_rate = float(fp_flows / (total_flows + 1e-9) * 100.0)
    return fp_rate, total_flows

def main(args):
    log.info("=" * 60)
    log.info("TRAINV9D — XGBoost + CTU-13 Supervised + Pure Thursday CORAL (NO CLASS WEIGHTS)")
    log.info("=" * 60)
    ...
    log.info("\n[2/6] Loading target domain data for CORAL (ONLY Thursday from data/eve/eve.json)...")
    X_target = load_unlabeled_target(args.target_eve, CORAL_TARGET_THURSDAY_SAMPLES)
    ...
    log.info("\n[5/6] StratifiedGroupKFold Benchmark Evaluation & XGBoost Training (NO WEIGHTS)...")
    ...
    for fold, (train_idx, val_idx) in enumerate(sgkf.split(X_train_s, y_source, groups=src_ips)):
        # ISOLATION: NO SAMPLE WEIGHTS (weight=None)
        dtr = xgb.DMatrix(X_train_s[train_idx], label=y_source[train_idx])
    ...
    log.info("\n[6/6] Optimizing binary threshold and evaluating FP rate (Low-RAM Chunked Mode)...")
    ...
    if args.target_eve.exists():
        thursday_fp_rate, total_flows = evaluate_fp_rate_chunked(bst, scaler, coral, best_thr, args.target_eve)
        log.info(f"  CICIDS2017 EVE FP Rate (@ {best_thr:.4f}): %{thursday_fp_rate:.4f} ({total_flows:,} flows)")
    ...
    log.info("\n" + "=" * 80)
    log.info("  TRAINV9D FINAL VERIFIED SUMMARY")
    log.info("=" * 80)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="trainv9d — XGBoost + CTU-13 Supervised + Pure Thursday CORAL (NO WEIGHTS)")
