...
class FlowEnrichment:
    ...
    Flow eventi geldiginde, ilgili enrichment alinir ve onbellekten silinir."""
    ...
    def ingest(self, event: dict):
        ...
        if fid is None or len(self._store) >= self._max:
            return
...
# ============================================================================
class IPBuffer:
    def __init__(self, window_size=WINDOW_SIZE, timeout_s=BUFFER_TIMEOUT_S):
        self.window_size   = window_size
        self.timeout_s     = timeout_s
        self.buffers       = {}    # src_ip -> deque(maxlen=window_size)
        self.last_seen     = {}    # src_ip -> datetime
        self.event_counter = 0

    def add(self, src_ip, feature_vector, timestamp=None):
        if src_ip not in self.buffers:
            self.buffers[src_ip] = deque(maxlen=self.window_size)
        self.buffers[src_ip].append(feature_vector)
        self.last_seen[src_ip] = timestamp or datetime.now()
        self.event_counter += 1
        if self.event_counter % CLEANUP_INTERVAL == 0:
            self.cleanup()
    ...
    def cleanup(self):
        ...
        if expired:
            print(f"  [cleanup] {len(expired)} IP silindi")
...
def load_xgboost(path: Path):
    ...
    # CUDA olmayan ortamda (Raspberry Pi) hata vermemesi icin
    try:
...
def hybrid_predict(event, enrichment,
                   ...
                   ip_buffer):
    """Tek bir flow event'i isler: XGBoost → (DoS/DDoS ise dur) → IPBuffer → LSTM → Alert"""
    ...
    #    XGBoost bu siniflari iyi tanıyor; LSTM bu siniflari gormedigi icin
    #    yanlis etiket verebilir. Direkt alarm uret.
    ...
    # ── 3) Diger saldirilar → IPBuffer'a ekle ───────────────────────────────
    ip_buffer.add(src_ip, features[0], timestamp=datetime.now())

    # ── 4) LSTM penceresi dolu mu? ──────────────────────────────────────────
    window = ip_buffer.get_window(src_ip)

    if window is not None and lstm_model is not None:
        # 70 ozellik × 40 akis penceresi → normalize → LSTM
