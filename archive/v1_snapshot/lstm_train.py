...
def extract_features_v7(event: dict, enrichment: dict) -> np.ndarray:
    ...
    enriched=np.zeros(N_FEATURES_v7-N_FEATURES_BASE,dtype=np.float32)
        ...
        for line in f:
            ...
            if etype == "flow":
                ...
                if feat_ts is not None:
                    ...
                    flows.append((fname_key, src_ip, ts, feat, label_id))
                    count += 1
...
def group_and_sort(flows):
    ...
    for fname, src_ip, ts, feat, label in flows:
        gid = (fname, src_ip)
        groups[gid].append((ts, feat, label))
    for gid in groups:
...
def make_windows(sessions, group_id):
    ...
    for sess in sessions:
        ...
        for i in range(0, n - WINDOW_SIZE + 1, STRIDE):
            ...
            # Filtre: Benign pencereler atilir
            if all(l == 0 for l in labels):
                continue
            X_seq = np.stack([item[1] for item in win])
