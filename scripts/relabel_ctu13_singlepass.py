    ...
    def __init__(self, label_path: Path):
        ...
        with open(label_path, "r", encoding="utf-8", errors="replace") as f:
            for line in f:
                ...
                lbl = parts[14].strip()
                if (saddr, sport) <= (daddr, dport): k = (saddr, sport, daddr, dport, proto)
                else: k = (daddr, dport, saddr, sport, proto)
                self.idx[k].append((ts_epoch, lbl))
                
    def match(self, sip, sport, dip, dport, proto, ts, win=5.0):
        proto = normalize_proto(proto)
        if (sip, sport) <= (dip, dport): k = (sip, sport, dip, dport, proto)
        else: k = (dip, dport, sip, sport, proto)
...
def main():
    ...
    for sc_dir in sorted(ctu_dir.glob("scenario_*")):
        ...
        if not eve_path.exists() or not label_files: continue
    ...
    print("=" * 80)
    print(f"  Combined Output File : {comb_path} ({len(comb_lines):,} lines)")
