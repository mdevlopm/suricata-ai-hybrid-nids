...
def parse_ts_pure_naive(ts_str: str) -> float:
...
def main():
    ...
    comb_f = open(comb_path, "w", encoding="utf-8")

    tot_sur, tot_mat, tot_bot = 0, 0, 0
    report = []
    start_all = time.time()
    ...
    for sc_dir in sorted(ctu_dir.glob("scenario_*")):
        ...
        with open(l_files[0], "r", encoding="utf-8", errors="replace") as f:
            for line in f:
                ...
                ts_ep = parse_ts_pure_naive(parts[0])
                if ts_ep <= 0: continue
                proto = parts[2].strip()
                saddr = parts[3].strip()
        ...
        with open(eve_path, "r", encoding="utf-8", errors="replace") as in_f:
            for line in in_f:
                ...
                ts_ep = parse_ts_pure_naive(ts_str)
        ...
        sf = open(sc_bot_path, "w", encoding="utf-8")
    ...
    print("=" * 80)
    print(f"  Combined Output File : {comb_path} ({tot_bot:,} lines)")
