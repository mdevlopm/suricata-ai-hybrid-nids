...
# -*- coding: utf-8 -*-
"""
Rebuild CTU-13 Ground Truth Botnet Dataset from the 13 scenarios
Using *.binetflow ground truth files and data/eve/CTU-13/scenario_*/eve.json
"""
...
def main():
    ...
    for sc_dir in sorted(ctu_dir.glob("scenario_*")):
        ...
        # Explicitly search for .binetflow text CSV files (avoiding binary .biargus)
        ...
        print(f"[{sc_name}] (file={binet_file.name}) Suricata={s_tot:,} | Matched={s_mat:,} ({pct:.2f}%) | Botnet Saved={s_bot:,} ({time.time()-t0:.1f}s)")
