...
def main():
    ...
    for sc_dir in sorted(ctu_dir.glob("scenario_*")):
        ...
        det_files = list(sc_dir.glob("**/detailed-bidirectional-flow-labels/*.binetflow")) + list(sc_dir.glob("**/detailed-bidirectional-flow-labels/*.biargus"))
        if det_files:
            l_files = det_files
        else:
            l_files = list(sc_dir.glob("*.binetflow")) + list(sc_dir.glob("*.biargus")) + list(sc_dir.glob("**/*.binetflow")) + list(sc_dir.glob("**/*.biargus"))
        ...
        with open(l_files[0], "r", encoding="utf-8", errors="replace") as f:
            for line in f:
                ...
                ts_ep = parse_pure_datetime(parts[0])
                if ts_ep <= 0: continue
                proto = parts[2].strip()
                saddr = parts[3].strip()
        ...
        with open(eve_path, "r", encoding="utf-8", errors="replace") as in_f:
            for line in in_f:
                ...
                ts_ep = parse_pure_datetime(ts_str)
        ...
        print(f"[{sc_name}] (file={l_files[0].name}) Suricata={s_tot:,} | Matched={s_mat:,} ({pct:.2f}%) | Botnet Saved={s_bot:,} ({time.time()-t0:.1f}s)")
