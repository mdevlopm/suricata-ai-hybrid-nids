...
def parse_ts(ts_str: str) -> float:
    ...
    try:
        ...
        clean_ts = clean_ts[:19].replace("/", "-").replace(" ", "T")
        dt = isoparse(clean_ts)
        micros = 0.0
        if "." in ts_str:
            dot_p = ts_str.find(".")
            m_str = ts_str[dot_p+1:dot_p+7]
            m_digits = "".join([c for c in m_str if c.isdigit()])
            if m_digits: micros = float(f"0.{m_digits}")
...
def main():
    ...
    comb_f = open(comb_path, "a", encoding="utf-8")
    ...
    for sc_dir in sorted(ctu_dir.glob("scenario_*")):
        ...
        if not eve_path.exists() or not l_files:
            continue
        ...
        with open(eve_path, "r", encoding="utf-8", errors="replace") as in_f, \
             open(sc_bot_path, "w", encoding="utf-8") as sf:
            for line in in_f:
                line = line.strip()
                if not line:
                    continue
    ...
    print("=" * 80)
    print(f"  Combined Output File : {comb_path} ({total_comb_saved:,} lines)")
