...
# -*- coding: utf-8 -*-
"""
Pipeline Train V10: Multi-Source Supervised IDS Training
Includes:
- CICIDS2018 relabeled flows (Benign, DoS, DDoS, WebAttack, Infiltration, Bot)
- CTU-13 verified Suricata Botnet flows (210,102 flows)
- CICIDS2017 Friday verified Suricata flows (Botnet, DDoS, PortScan, Benign)
No CORAL, No Artificial Distortions. Pure Multi-Source Supervised Representation.
"""
...
MODEL_OUT = BASE_DIR / "models/ids_model_v10_final.pkl"
...
def main():
    print("=" * 80)
    print("IDS PIPELINE V10: MULTI-SOURCE SUPERVISED MODEL TRAINING")
    ...
    for cname in CLASS_MAP:
        ...
        arr = load_json_flows(p, max_samples=samples_2018[cname])
        X_dict[cname].append(arr)
        print(f"  • CICIDS2018 {cname:<12}: {len(arr):,}")

    # B. CTU-13 Real Suricata Botnet Flows
    print("\n[2/4] Loading CTU-13 Verified Botnet Flows...")
    ctu_bot_path = DATA_CTU13 / "eve_Bot.json"
    arr_ctu = load_json_flows(ctu_bot_path, max_samples=60_000)
    X_dict["Bot"].append(arr_ctu)
    print(f"  • CTU-13 Botnet Flows       : {len(arr_ctu):,}")

    # C. CICIDS2017 Friday Flows (if available)
    if DATA_2017.exists():
        ...
        arr_17_benign = load_json_flows(p_2017_benign, max_samples=40_000)
        ...
        if len(arr_17_pscan) > 0:
            X_dict["DoS"].append(arr_17_pscan) # Group PortScan with DoS/Recon
    ...
    print("\nTraining XGBoost V10 Multi-Class Classifier...")
    params = {
    ...
    bundle = {
        ...
        'scaler': scaler,
        'class_map': CLASS_MAP,
        'coral_adapter': None, # PURE SUPERVISED
        'threshold': 0.84,
        'version': 'v10_multi_source',
        'timestamp': datetime.now().isoformat()
    }
    MODEL_OUT.parent.mkdir(parents=True, exist_ok=True)
    with open(MODEL_OUT, 'wb') as f:
        pickle.dump(bundle, f, protocol=pickle.HIGHEST_PROTOCOL)
    print(f"Saved V10 Model to: {MODEL_OUT}")

    # 5. Dual-Metric Operating Curve Evaluation (0.50 - 0.99)
    print("\n" + "=" * 105)
    print("OPERATING CURVE EVALUATION (0.50 - 0.99) FOR V10 MODEL")
    print("=" * 105)
    ...
    with open('/tmp/v10_operating_curve.json', 'w') as f:
