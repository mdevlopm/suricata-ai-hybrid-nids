...
# ── FOCAL LOSS (multi-class, gamma=2.0) ────────────────────────────────────
def focal_loss(gamma=2.0, alpha=None):
    """
    Focal Loss (Lin et al. 2017), multi-class versiyonu.
    - gamma=2.0: kolay orneklerin loss katkisini azaltir
    - alpha=None: class_weight ayri parametre olarak verilir (model.fit)
    - alpha=liste/array: per-class alpha (opsiyonel, class_weight ile birlikte kullanilmaz)
    """
    ...
    def loss(y_true, y_pred):
        ...
        if alpha is not None:
            ...
            return alpha_t * (1.0 - pt) ** gamma * ce
...
def train():
    ...
    # --- 5) Sinif agirliklari (Focal Loss ile birlikte kullanilir, Faz-1) ---
    ...
    with strategy.scope():
        model = build_lstm()
        # CosineDecay: ReduceLROnPlateau yerine, her adimda LR'yi kosinus egrisiyle dusurur
        total_steps = (len(y_tr) // 256 + 1) * 50
    ...
    print(f"\n  Egitim basliyor... (Focal Loss gamma=2.0 + class_weight + Cosine LR)")
    print(f"  Train: {len(y_tr):,}  Val: {len(y_val):,}")
    t0 = time.time()
    history = model.fit(
        X_tr, y_tr,
    ...
    metadata = {
        ...
        "architecture"     : "BiLSTM(64)+LSTM(32)+Dense(32)",
        "loss"             : "FocalLoss(gamma=2.0) + class_weight(balanced)",
        "lr_schedule"      : "CosineDecay(1e-3, alpha=1e-4)",
