...
DEFAULT_XGB_MODEL   = Path(__file__).parent.parent / "models" / "baseline" / "ids_model_v10c_baseline.pkl"
...
# Module-level metric for CORAL failures
coral_failure_count = 0


def get_coral_failure_count() -> int:
    """Returns the total number of runtime CORAL transformation failures."""
    return coral_failure_count


def validate_coral_adapter(coral_adapter, feature_dim: int = 70):
    """
    Pipeline başlangıcında (process start) coral_adapter'ı TEK bir test transform'u ile doğrular.
    Başarısız olursa pipeline'ı başlatmaz, net bir RuntimeError ile sonlandırır.
    """
    if coral_adapter is None:
        return
    print("  [CORAL Test] Başlangıç doğrulama testi yapılıyor...")
    try:
        dummy_target = np.zeros((1, feature_dim), dtype=np.float32)
        transformed = coral_adapter.transform_target(dummy_target)
        if transformed is None or transformed.shape != (1, feature_dim):
            raise ValueError(f"CORAL transform çıktısı geçersiz şekle sahip: {transformed.shape if transformed is not None else None}")
        print("  [CORAL Test] ✅ Startup test transform başarılı (Target -> Source alignment OK)")
    except Exception as e:
        log.error("CRITICAL: CORAL adapter başlangıç doğrulama testi başarısız! Pipeline başlatılamadı: %s", e, exc_info=True)
        raise RuntimeError(f"CORAL adapter startup validation failed: {e}") from e


# ============================================================================
# MODEL YUKLEME
# ============================================================================
def load_xgboost(path: Path, enable_coral: bool = False):
    ...
    if enable_coral:
        coral_adapter = b.get("coral_adapter")
        print(f"    CORAL     : ETKİN (Target domain alignment aktif)")
    else:
        coral_adapter = None
        print(f"    CORAL     : DEVRE DIŞI (Güvenli Raw XGBoost modu - Recall=%98.57 korumalı)")
    ...
    # Pipeline başlangıcında CORAL adapter doğrulaması (Startup Test)
    if coral_adapter is not None:
        feat_dim = scaler.mean_.shape[0] if hasattr(scaler, 'mean_') else 70
        validate_coral_adapter(coral_adapter, feature_dim=feat_dim)
    ...
    try:
        ...
        pass
    return model, scaler, thresh, inv_cmap, cmap, is_multiclass, coral_adapter


def load_lstm(model_path: Path, scaler_path: Path, meta_path: Path):
    if tf is None:
        print("  !! TensorFlow yüklü değil -> sadece XGBoost modunda çalış")
...
def hybrid_predict(event, enrichment,
    ...
    if coral_adapter is not None:
        try:
            ...
            coral_failure_count += 1
            log.error("CORAL domain adaptation transform hatası (toplam hata count=%d): %s", coral_failure_count, e, exc_info=True)

    # ── 1) XGBoost ──────────────────────────────────────────────────────────
    scaled_feat = xgb_scaler.transform(features)
    if hasattr(xgb_model, "predict_proba"):
        xgb_prob = xgb_model.predict_proba(scaled_feat)[0]
    else:
        import xgboost as xgb
        xgb_prob = xgb_model.predict(xgb.DMatrix(scaled_feat))[0]
...
Ornekler:
  ...
  Elle:    python3 hybrid_inference.py \\
    ...
    parser.add_argument("--enable-coral", action="store_true", default=False,
                        help="CORAL domain adaptation'ı aktif eder (varsayılan: devre dışı)")
    args = parser.parse_args()

    # XGBoost yukle
    xgb_model, xgb_scaler, xgb_threshold, inv_cmap, cmap, is_multiclass, coral_adapter = \
        load_xgboost(Path(args.xgb_model), enable_coral=args.enable_coral)
