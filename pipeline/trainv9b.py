# -*- coding: utf-8 -*-
"""
trainv9b.py — XGBoost IDS Training with CTU-13 Supervised + Pure Thursday CORAL
=============================================================================
ISOLATING THE EFFECT OF CTU-13 BOT FLOWS:
  1. Supervised Source Dataset: CICIDS2018 (relabeled) + CTU-13 (24,702 Bot flows) -> Total Bot = 174,702
  2. CORAL Target Domain: ONLY CICIDS2017 Thursday (50,000 flows). NO MCFP.
  3. Hyperparameters: IDENTICAL to trainv8.py (max_depth=8, eta=0.05, subsample=0.80, colsample_bytree=0.80, min_child_weight=3)
  4. Parallel Execution: nthread=8 for fast multi-core tree construction
  5. Benchmark Evaluation: StratifiedGroupKFold(group=src_ip) 5-fold Macro F1 + Thursday FP Rate
"""
...
DEFAULT_OUT_MODEL = BASE_DIR / "models/ids_model_v9b_final.pkl"
...
def load_unlabeled_target(thursday_path: Path, max_thursday: int):
    ...
    if thursday_path.exists():
        ...
        log.info(f"  Target CORAL Thursday ONLY: {len(t_feats):,} samples")
...
def main(args):
    log.info("=" * 60)
    log.info("TRAINV9B — XGBoost + CTU-13 Supervised + Pure Thursday CORAL")
    log.info("=" * 60)
    ...
    log.info("\n[2/6] Loading target domain data for CORAL (ONLY Thursday)...")
    ...
    coral = CORALDomainAdapter(lambda_reg=1e-5)
    ...
    log.info("\n" + "=" * 80)
    log.info("  TRAINV9B FINAL VERIFIED SUMMARY")
    log.info("=" * 80)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="trainv9b — XGBoost + CTU-13 Supervised + Pure Thursday CORAL")
